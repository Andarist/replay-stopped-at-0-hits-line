...
interface TypegenMeta extends TypegenEnabled {
  // having this property here breaks the inference for the `TypegenDisabled` case
...
type ActionFunction<TEvent extends EventObject> = (event: TEvent) => void;
...
type GenerateActionsConfigPart<TEvent extends EventObject, TTypesMeta> = {
...
interface Model<TEvent extends EventObject> {
  createMachine: {
    <TTypesMeta extends TypegenConstraint = TypegenDisabled>(
      ...
      implementations?: MaybeTypegenMachineOptions<TEvent, TTypesMeta>
    ): void;
  };
}
